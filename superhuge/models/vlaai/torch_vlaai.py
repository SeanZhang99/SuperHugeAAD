...
def extractor_layer(
    ...
    return nn.Sequential(
            ...
            nn.Conv1d(input_channels, num_kernels, kernel_size, padding=0),
            nn.GroupNorm(num_groups=1, num_channels=num_kernels), 
            nn.LeakyReLU(),
    )
...
def extractor(
    ...
    layers = []
    for i, (out_ch, kernel_size) in enumerate(zip(num_kernels, kernel_sizes)):
        layers.append(
            extractor_layer(
                input_channels=in_ch if i == 1 else out_ch,
                num_kernels=out_ch,
...
def output_context(
    ...
    return nn.Sequential(
        nn.ConstantPad1d((kernel_size - 1, 0), 0),
        nn.Conv1d(input_channels, num_kernel, kernel_size, padding=0),
        nn.GroupNorm(num_groups=1, num_channels=num_kernel), 
        nn.LeakyReLU(),
    )
...
class VLAAI(nn.Module):
    ...
    def __init__(
        ...
        self.output_context_model = output_context(**output_context_args)

        dense_in_features = output_context_args["num_kernel"]
